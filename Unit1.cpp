...
void SaveUsersListToIni(const String userTG[], const String userTGID[], int count)
{
    ...
    try
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            usersList += userTG[i] + (i < count - 1 ? ", " : ""); // Добавляем запятую, если это не последний элемент
            usersListId += userTGID[i] + (i < count - 1 ? ", " : ""); // То же для ID
        }

        // Записываем значения в секцию [TELEGRAMMSETTING]
...
void LoadUsersListFromIni(String userTG[], String userTGID[], int &count)
{
    ...
    try
    {
        // Читаем данные из секции [TELEGRAMMSETTING]
        ...
        count = users->Count; // Устанавливаем количество загруженных пользователей
		...
		if(ini->ReadBool("TELEGRAMMSETTING","SendMessage",false) == true)
		{
			...
			if(ini->ReadDate("TELEGRAMMSETTING","LastDateSendMessage",Now()) < Date())
			{
					...
					for (int i = 0; i < countTgUsers; ++i)
					{
						  ...
						  {
							   String ending = GetEnding(StrToInt(prosrochka), "обращение", "обращений", "обращения");
							   ending1 = GetEnding(StrToInt(prosrochka), "просроченное", "просроченных", "просроченных");

							   message = userTG[i]+" ВНИМАНИЕ! У вас имеется: "+ prosrochka + " "+ ending1 + " " + ending + " срочно свяжитесь с общим отделом.";
						  ...
						  {
								message = message + " Истекает срок  " + istekaet +" "+ GetEnding(StrToInt(istekaet), "обращение", "обращений", "обращения");
								SendTelegramMessage(ini->ReadString("TELEGRAMMSETTING","BotToken",""), userTGID[i], message);
						  }
